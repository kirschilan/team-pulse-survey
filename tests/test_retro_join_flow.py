...
with sync_playwright() as p:
    ...
    # SEC-2: the session doc's id is now the relay ROOM id (a one-way hash
    # of the session's real secret -- crypto.js's roomIdFor()), never a
    # human-typeable code -- see relay-client.js's secretForRoom() below for
    # how a device that started/co-facilitated this session recovers the
    # real secret its join/co-facilitate links need.
    print("session id looks like a room id, not a typeable code:", session_id, len(session_id))
    assert len(session_id) == 16
    secret = page.evaluate("SquadPulseRelay.secretForRoom(%r)" % session_id)
    print("session secret (never shown in the UI):", secret)
    assert secret and secret != session_id
    ...
    # SEC-2 (STATUS.md's locked PO decision): there's no raw code display on
    # the session card any more -- a retro session is joined only by
    # scanning its QR code or opening its link.
    print("no raw session code shown anywhere on the card:", page.query_selector('.session-code'))
    assert page.query_selector('.session-code') is None

    # ---- the join link/QR is shown directly, no longer collapsed behind a
    # secondary "Or scan/share a link" <details> fallback ----
    join_share_open = page.eval_on_selector('.join-share-block', 'el=>!!el')
    print("join-share block present and not a collapsed <details>:", join_share_open)
    assert join_share_open
    assert page.query_selector('.join-share-block details') is None
    ...
    print("security notice next to the join link:", security_notice)
    assert "anyone" in security_notice.lower() and "secure channel" in security_notice.lower()

    # the QR was already rendered directly into the (always-visible) markup
    # when the session card first rendered -- no <details> to expand any more.
    ...
    secret2 = page.evaluate("SquadPulseRelay.secretForRoom(%r)" % sid)

    # ============ participant device A: join via the ?session=<secret> link (SEC-2: the ONLY path now) ============
    ...
    page2.goto("file://" + str(join_out.resolve()) + "?session=" + secret2)
    # enterJoinMode()'s view-hidden toggles happen synchronously at boot,
    # but the join card's actual CONTENT depends on state.live flipping,
    # roomIdFor()'s real crypto.subtle digest resolving, and the session doc
    # arriving via the fake store's onSnapshot (real, if short, async gaps)
    # -- wait for that real end state, the same one the assertions below
    # actually depend on, rather than guess.
    page2.wait_for_selector('#joinCard .direct-row', state="attached")

    print("=== participant device A (join via link) ===")
    ...
    # ============ participant device B: a fresh, plain load (no ?session=) offers NO manual join UI at all (SEC-2) ============
    # Before SEC-2 this was the "fix for the iPhone bug" primary path (a
    # header button + typed-code modal) -- the PO decision retired that
    # premise entirely (STATUS.md): a retro session is joined ONLY by
    # scanning its QR code or opening its link now, so a device that never
    # received one just sees the normal board, nothing more.
    plain_out = build_page(out_name="_test_retro_join_deviceB.html")

    page3 = browser.new_page(viewport={"width":420,"height":900})
    errors3 = []
    page3.on("pageerror", lambda e: errors3.append(str(e)))
    page3.goto("file://" + str(plain_out.resolve()))  # NOTE: no ?session= query string at all
    page3.wait_for_selector('.view-switch', state="attached")

    print("=== participant device B (plain load, no invitation) ===")
    print("view-switch visible (normal board, not join mode):", page3.eval_on_selector('.view-switch', 'el => el.hidden') == False)
    assert page3.eval_on_selector('.view-switch', 'el => el.hidden') == False
    print("no join-code entry point exists anywhere in the header:", page3.query_selector('#joinCodeBtn'))
    assert page3.query_selector('#joinCodeBtn') is None
    print("errors (plain-load device B):", errors3)

    page3.screenshot(path=str(test_output_path("shot_retro_join_deviceB.png")), full_page=True)

    # ============ bad/expired secret, via a link (the only join path now) ============
    ...
    page4.goto("file://" + str(badcode_out.resolve()) + "?session=ZZZZZZ-bad-secret")
    # A bad secret never gets a `.direct-row` to wait on -- the real signal
    # here is the join card's own placeholder text finally changing away
    # from "Connecting..." once roomIdFor() resolves and the (not-found)
    # lookup completes.
    ...
    print("=== bad/nonexistent secret, via a link ===")
    print("heading:", heading4)
    assert "isn" in heading4.lower() and "open" in heading4.lower()
    print("errors (bad secret device):", errors4)
    ...
    print("=== ALL ERRORS: SM=", errors, "joinA(link)=", errors2, "joinB(plain load)=", errors3, "badsecret=", errors4, "badlink=", errors5)
    browser.close()
