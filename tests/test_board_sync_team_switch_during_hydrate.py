    ...
    with sync_playwright() as p:
        ...
        result = page.evaluate("""
          async ([team1Secret, team2Secret]) => {
              ...
              if (path === "boards/" + team1RoomId) {
                ...
                ref.get = function(){
                  return new Promise(function(resolve){
                    setTimeout(function(){ resolve(origGet()); }, 300);
                  });
                };
              }
              return ref;
            ...
            var staleHydrate = hydrateFromTeamIfConnected();  // won't resolve for 300ms

            // Switch away from Team 1 to Team 2 well before that resolves.
            connectWithSecret(team2Secret);
            // connectWithSecret's own hydrate for Team 2 is NOT patched/delayed
            // -- give it (and its push+subscribe chain) real time to finish
            // before the stale Team 1 hydrate we're racing against does.
            await new Promise(function(resolve){ setTimeout(resolve, 150); });
            var afterSwitchName = state.squads.filter(function(s){ return s.id==="squad-1"; })[0].name;

            // Now let the deliberately-delayed, now-STALE Team 1 hydrate
            // actually finish resolving in the background.
            await staleHydrate;
