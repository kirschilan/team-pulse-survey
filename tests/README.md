...
| `test_idle_tab_sync_loop.py` | PERF-1 (STATUS.md's "Runtime performance backlog"): two tabs of the same browser sharing localStorage, both team-synced and genuinely idle, must not fall into a self-sustaining sync loop (render/push/remote-apply counters stay flat, evaluate() round trips stay fast) — while a real edit still propagates live between those two tabs AND to a completely separate browser context, and survives a reload. Uses the real `local-store.js`/relay (`write_plain_index()`), not `fake_store.html`, since this bug lives specifically in the native `storage` event two same-origin tabs share |
| `test_board_sync_revert_after_remote_change.py` | Codex review fix on PR #15 (the SEC-1/PERF-1 port): PERF-1's upload-dedup baseline only tracked what THIS device itself pushed, never what a remote snapshot (applied via the live subscription) actually changed local content to — so reverting a value back to what this device pushed BEFORE receiving a teammate's live change looked like "nothing changed" and was silently skipped. Two real devices, relay only: A sets "Original", B sets "Remote change" and A receives it live, A reverts to "Original" — must reach B, not get stuck |
| `test_render_batching_on_multi_doc_apply.py` | PERF-2 (STATUS.md's "Runtime performance backlog"): a multi-doc write (a remote board snapshot applying N squads + M dimensions, or a local starter-template load) fired `renderAll()` once per INDIVIDUAL doc written, not once for the whole batch — confirmed 31 renders for a 3-squad/12-dimension apply, 66 for 40/25, scaling with board size. Proves the fix (skip rendering while `hydrating`/`suppressingLocalRewrite` is set, render once after) collapses both to exactly 1, that an ordinary single edit still renders normally, and records real per-call timing (tens of milliseconds for one `renderAll()` pass at 40 squads/25 dimensions). Installs its render-counting wrapper exactly once per page (Codex review fix on PR #17 — re-installing it before every measurement double-/triple-counted later ones) |
...
| `test_security_headers.py` | SEC-3: the CSP `<meta>` tag in `index.html` is present with the directives the app actually needs (`script-src 'self'`, no `'unsafe-inline'` for scripts, `ws:`/`wss:` in `connect-src` for the deployment-configurable relay, no `frame-ancestors` — that's a deliberately deferred PO decision, see STATUS.md), then drives a realistic walkthrough (boot, language switch, About, start a session, render its QR, load a starter template, join as a participant) listening for the browser's own `securitypolicyviolation` events and asserting zero fired. Verified the assertions aren't vacuous by temporarily breaking `connect-src` and confirming the test catches it. See `tests/unit/test_security_headers.js` for the vercel.json half (`X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy` — no meta-tag equivalent, so only testable as JSON config, plus the same deliberate absence of `X-Frame-Options`) |
