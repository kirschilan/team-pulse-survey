...
with sync_playwright() as p:
    ...
    for view in ['tribe', 'squad', 'admin']:
        ...
        page.locator('#aboutCloseBtn').focus()
    ...
    page.click('#aboutHelpBtn')
    page.click('#aboutIntro')
    expect(page.locator('#aboutDialog')).to_be_visible()
    box = page.locator('#aboutIntro').bounding_box()
    ...
    expect(page.locator('#aboutHelpBtn')).to_be_focused()
    page.click('#aboutHelpBtn')
    for guide in ['participant', 'facilitator', 'results']:
        page.click('#about-' + guide + ' summary')
    ...
    expect(page.locator('#about-participant summary')).to_have_text('מצטרפים לרטרוספקטיבה?')
    expect(page.locator('#about-facilitator summary')).to_have_text('מנחים רטרוספקטיבה?')
    expect(page.locator('#about-results summary')).to_have_text('איך לקרוא את התוצאות')
    ...
    page.click('#aboutCloseBtn')
    page.evaluate("setLocale('en'); state.joinSessionId = 'TEST12'; enterJoinMode(); state.joinDraftAnswers = {release:'good'}")
