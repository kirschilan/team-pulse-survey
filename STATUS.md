...
- 2026-09-18 — Picked up REF-7 through REF-10 (STATUS.md's "Repository / tooling improvements"
  ...
  `ref-8-relay-ci-npm-ci`, PR opened against `claude/optimistic-keller-holuql`. Both `unit-and-relay`
  and `playwright` jobs now run `npm ci` in `relay/` instead of `npm install`; verified locally
  (`rm -rf relay/node_modules && npm ci` succeeds against the committed lockfile, relay's own suite
  passes under it). `relay/README.md` now explains the CI-vs-local distinction (`npm ci` for a
  byte-for-byte reproduction, `npm install` when actually adding/upgrading a dependency locally)
  right next to its own existing `npm install` instructions, rather than leaving a reader to wonder
  why the workflow file says something different. Docs/CI-only; no relay code changes. Full suite
  re-verified green: 180/180 unit tests, all 63 Playwright files, relay's own suite.
