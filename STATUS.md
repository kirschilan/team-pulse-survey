...
- **Preview and Production share one Render relay instance, not one each — accepted tradeoff, not
  a gap (PO reconfirmed 2026-09-18).** Two real, currently low-stakes consequences: (1) no isolated
  place to test a `relay/server.js`/wire-protocol change against real traffic patterns before it's
  live for Production users — every Preview deploy shares the exact relay process Production is on;
  (2) a runaway Preview branch (e.g. a reconnect-storm bug like the one SEC-1's rate limits now
  guard against) could degrade the relay for Production too, since there's no resource isolation
  between environments. Both are non-issues at this app's current scale (a single team's internal
  tool, SEC-1's per-IP/per-room/global bounds already apply regardless of which environment a
  connection comes from). Fix, whenever it stops being a non-issue: a second free Render instance +
  a second `SQUAD_PULSE_RELAY_URL` scoped to Preview in Vercel's project settings — cheap, not worth
  doing preemptively.
...
**2026-09-18 correction:** two rows this table used to carry are gone, not just re-worded.
**Save-board / Load-board-to-file** wasn't actually a gap any more — it was already built and
shipped as the JSON board export/import feature (Multi-language rollout backlog Story 13,
`public/js/board-export.js`/`board-import-*.js`: a real `window.claude.use("downloads")` file save
and a real `FileReader`-based file load, round-tripping squads/ratings/dimensions/templates/
settings, later extended by RETRO-1 to also carry a squad's last finished retro). The row had gone
stale the moment that feature landed and nobody removed it. **Embedding decision (subdomain+iframe
vs. same-site route)** was a duplicate of the "Security hardening backlog"'s SEC-3a row above
(same YAGNI/PO-gated call, more detail there) — consolidated into one entry instead of two that
could drift out of sync.

| Not built | Why it's cut for now | What would trigger building it |
|---|---|---|
| Relay deployed on Vercel itself (one deployment, not two) | Deliberately rejected, not just deferred — see the locked decision above and `relay/README.md`'s "Why not a Vercel Function". Reconfirmed by the PO, 2026-09-18: "we have a free working solution." | Only if Vercel's WebSocket support later guarantees same-instance routing without an external store, which would remove the reason this was rejected |
...
| P2/P3 | REF-13 — Add ESLint, the one concrete repeatable-tool recommendation from the Copilot review (below) that this review deliberately held back | **Problem**: no static-analysis pass existed in this repo at all before this — every finding above came from manual review, not a repeatable check. **Value**: real bugs a human reviewer can miss on a large diff (a wrong-context `.hasOwnProperty()` call, a `var` silently escaping the block it looks scoped to) get caught automatically, on every push, not just when someone happens to look. **Acceptance criteria**: a real lint config, not a stub; wired into CI so it actually runs; first real run's findings triaged (fixed if real, justified if a deliberate pattern) rather than the config tuned to pass silently. | **DONE (2026-09-18)**, PO call: "do it now." `eslint.config.js` (flat config) + root `package.json`/`package-lock.json`, wired into `.github/workflows/tests.yml`'s `unit-and-relay` job. Three rule groups, not one uniform config, because the codebase genuinely has three different sharing models: `public/js/*.js`/`public/app.js` are classic `<script>` files that deliberately share one global scope (see "The app's file layout" above — Chromium blocks cross-file `import` over `file://`), so `no-undef` is off there (ESLint lints one file at a time and can't see a sibling file's declarations — leaving it on is ~14 files' worth of false positives) and `no-unused-vars` only checks nested/local scope, not top-level declarations (mirror-image reason: a function that looks unused in its own file is routinely called from another one). `relay/`, `scripts/`, and `tests/unit/` are ordinary Node CommonJS with no such split, so they keep the full ruleset. First real run found 4 genuine findings, all fixed (not suppressed): three separate `.hasOwnProperty()` calls made directly on a plain object instead of `Object.prototype.hasOwnProperty.call(obj, key)` (`helpers.js`'s `consolidateBand()`, `retro-join.js`'s draft-answer init, `dimensions.js`'s `i18nPanelHtml()`) — safe only because nothing today lets a dimension/draft key collide with an `Object.prototype` name, but JSON board import and template save/load both turn arbitrary user text into keys, so it's a real latent gap, not a style nit; and `relay/test/rate-limits.test.js` using `var` inside six independent `{ }` blocks, so `no-redeclare` correctly caught `port`/`wss`/`a`/`b`/`c`/`d` etc. silently reusing the same binding across blocks that visibly look separately scoped (harmless today — nothing crossed a block boundary — but not what the code visibly intends) — converted to `let`, restoring real block scoping. Left as warnings, not fixed: ~30 `catch(e)` sites where `e` is genuinely unused — the same "silent catch, no diagnostic trace" gap `docs/refactoring-report.md` already flagged and partially fixed; ESLint now keeps the remaining instances visible instead of hidden. Verified: 184-test unit suite and relay's own protocol/storage/rate-limit tests all re-run clean after every fix; Playwright itself wasn't runnable in the environment that added this (no `playwright` package pre-installed there), but neither fix changes behavior for any case the existing suite already exercises. **Does this cover REF-4/REF-5's `complexity`/`max-lines`/`max-depth`/`max-params` recommendation, or `docs/refactoring-report.md`'s open naming/abbreviation item?** No, on both counts, checked directly rather than assumed — those size/complexity rules weren't added this round (worth a follow-up now that REF-4/REF-5 have already produced the smaller, single-responsibility files to set real limits against), and ESLint can't perform the naming pass itself (that's a real, if mechanical, find-and-replace across ~3400+ lines with genuine regression risk if done carelessly). An `id-denylist`-style rule COULD stop new `sq`/`sess`/`dim` short-forms from creeping in prospectively, but a blanket denylist on `d` specifically would false-positive on every unrelated one-letter use elsewhere — not added; `docs/refactoring-report.md`'s own call (file-by-file, next time that file is touched for a real reason) still stands. | `eslint.config.js`, root `package.json`/`package-lock.json` (new), `.github/workflows/tests.yml`'s `unit-and-relay` job, plus the four small fixes above; lint-only otherwise, no runtime behavior change. |
...
recommendation from the original Copilot-assessment review — an ESLint pass (`complexity`,
`max-lines`, `max-depth`, `max-params` rules) to give REF-4/REF-5's targets a repeatable check going
forward — is now REF-13 above: ESLint itself landed 2026-09-18, but without those specific
size/complexity rules yet (see REF-13's own note). Still not proposed as an ongoing *tracked metric*
with an agreed action threshold — that stays a separate, undecided question even once the tool
...
- **Embedding/framing** (`frame-ancestors`/`X-Frame-Options`) — remains **YAGNI, PO-gated** per
  ...
  that decision.
- **ESLint config** — no longer deferred; see REF-13 above (**DONE, 2026-09-18**, PO call: "do it
  now"). Left here as a pointer so this section's own history stays readable, not because the item
  is still open.
