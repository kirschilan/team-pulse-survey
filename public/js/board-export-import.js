...
// Story 13, item 4: this file used to be csv.js, carrying both CSV export/
// import and the JSON board export/import that replaced it. CSV's own
// runtime code (toCSV()/exportBtn, and the whole parseCSV()/buildImportPlan()/
// applyImportPlan() import path) is deleted -- JSON is now the only board
// export/import format, so a name that still said "csv" was actively
// misleading about what this file does. Renamed to match, same content
// otherwise (verified with `git mv` + a content diff, not a rewrite).
...
// ---------- JSON board import (beta): squads, dimensions, templates & board settings ----------
// Story 13, items 3a (squads & ratings) + 3b (dimensions/templates/board
// settings) -- one file, one preview modal, one Merge/Replace toggle,
// governing whichever of the two scopes (Squads & ratings / Dimensions,
// templates & board settings) are checked.
//
...
// buildTemplateImportPlan()/renderJsonImportPreview() later. Loose on purpose:
