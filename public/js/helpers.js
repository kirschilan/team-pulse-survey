...
function joinUrlFor(secret){
  return window.location.origin + window.location.pathname + "?session=" + encodeURIComponent(secret) + langParamFor() + teamHashFor();
}
...
function coFacilitateUrlFor(secret){
  return window.location.origin + window.location.pathname + "?cofacilitate=" + encodeURIComponent(secret) + langParamFor() + teamHashFor();
}
